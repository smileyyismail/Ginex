...
interface SimpleAdminTableProps {
  ...
  onUpdate: (id: string, form: FormData) => Promise<{ success: boolean; error?: string }>;
...
export function SimpleAdminTable({
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (file) {
        uploadedUrl = await uploadImage(file, storageFolder);
      }
      ...
      form.append(imageField, uploadedUrl);
          ...
          <TableBody>
            {data.map((item) => (
              <TableRow key={item.id}>
                ...
                <TableCell className="text-text-secondary">{item.slug}</TableCell>
            ...
            {data.length === 0 && (
              <TableRow>
                <TableCell colSpan={4} className="text-center py-12 text-text-secondary">No {title.toLowerCase()} found.</TableCell>
              </TableRow>
            )}
          </TableBody>
        </Table>
